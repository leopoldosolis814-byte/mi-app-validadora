    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Ken Burns: imagen activa hace zoom lento */
        .slide-active img {
            ...
            <div className="card-main" style={{ padding: '28px' }}>
              ...
              >
                {cargandoSugerencias
                  ? '· Generando pivots ·'
                  : idea.trim()
                  ? '↻  Explorar 3 pivots de esta idea'
                  : '↻  Sugerirme 3 ideas para comenzar'}
              </button>

              {sugerencias.length > 0 && (
                <div style={{ marginTop: '16px', marginBottom: '4px' }}>
                  <p style={{ fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '10px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {sugerencias.map((s, i) => (
                      <button
                        key={i}
                        onClick={() => { setIdea(`${s.titulo}: ${s.problema}`); setSugerencias([]) }}
                        className="sugerencia-card"
                      >
                        <div className="sug-title">{s.titulo}</div>
                        <div className="sug-desc">{s.problema}</div>
                        <div className="sug-meta">— {s.cliente} &nbsp;·&nbsp; {s.validacion}</div>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {veredicto && (
                <div className="resultado-card">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    <span className="resultado-titulo">Análisis completo</span>
                    {postsAnalizados > 0 && (
                      <span className="badge-posts">{postsAnalizados} posts</span>
                    )}
                  </div>

                  <pre className="resultado-texto">{veredicto}</pre>

                  {fuentes.length > 0 && (
                    <>
                      <div className="divider" />
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>Fuentes</span>
                        {fuentes.map((f, i) => (
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginTop: '28px' }}>
              {IMAGENES_FONDO.map((_, i) => (
                <button
                  ...
                  style={{ width: i === indiceImagen ? '32px' : '16px' }}
                />
              ))}
            </div>
